...
def main():
    ...
    if not os.path.exists(gh): gh = "gh"
    ...
    rows = []
    for r in repos:
        rows.append({"name": r.get("name",""), "description": r.get("description") or "", "private": r.get("isPrivate",False), "default_branch": (r.get("defaultBranchRef") or {}).get("name",""), "language": (r.get("primaryLanguage") or {}).get("name",""), "updated_at": r.get("updatedAt",""), "stars": r.get("stargazerCount",0), "forks": r.get("forkCount",0), "open_issues": (r.get("issues") or {}).get("totalCount",0)})
